...
# Assumes that the gnomAD allele frequency has been added by vcfanno and is therfore in the INFO field
...
type
  key = tuple[sample:string, impact:Impact, gt:int8]
  score_key = tuple[sample:string, impact:Impact]
...
# define output files
var
  output = open(commandLineParams()[0]&"_variant_counts_result.txt", fmWrite)
  score_output = open(commandLineParams()[0]&"_variant_scores_result.txt", fmWrite)

# initizlie tables
var
  tbl = newCountTable[key](8192)
  score_tbl = initTable[score_key, int](8192)
...
for variant in vcf:
    ...
    # many variants will not be in gnomAD and are INCLUDED
    var
      gnomad_af = newSeq[float32]()
      status = variant.info.get("gnomAD_AF", gnomad_af)
    if status == Status.UndefinedTag:
      quit "unknown field"
    if status == Status.OK and gnomad_af[0] > gnomAD_AF_threshold:
      continue
    #echo variant, gnomad_af
